...
CostmapFilterInfoServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  update_keepout_zone_sub_ = this->create_subscription<std_msgs::msg::String>(
    "/update_zone", 10, std::bind(&CostmapFilterInfoServer::callbackUpdateKeepoutZone, this, _1));
...
void CostmapFilterInfoServer::callbackUpdateKeepoutZone(const std_msgs::msg::String::SharedPtr msg)
{
  // //��������
  std::string data = msg->data;
  std::unique_ptr<nav2_msgs::msg::CostmapFilterInfo> msg_callback;
  msg_callback = std::make_unique<nav2_msgs::msg::CostmapFilterInfo>();
  msg_callback->header.frame_id = "";
  msg_callback->header.stamp = now();
  msg_callback->type = get_parameter("type").as_int();
  msg_callback->filter_mask_topic = get_parameter("mask_topic").as_string();
  msg_callback->base = static_cast<float>(get_parameter("base").as_double());
  msg_callback->multiplier = static_cast<float>(get_parameter("multiplier").as_double());
  std::string keepout_filter = "/costmap_filter_info";
  rclcpp_lifecycle::LifecyclePublisher<nav2_msgs::msg::CostmapFilterInfo>::SharedPtr keepout_pub_ = this->create_publisher<nav2_msgs::msg::CostmapFilterInfo>(
    keepout_filter, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
  keepout_pub_->on_activate();
  keepout_pub_->publish(std::move(msg_callback));
  // keepout_pub_->publish(std::move(msg_));  
  RCLCPP_INFO(get_logger(), "CostmapFilterInfoServer,publish topic /costmap_filter_info %s",data.c_str());
}
